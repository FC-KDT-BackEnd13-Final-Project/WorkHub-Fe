...
export const historyApi = {
  ...
  getHistoriesByTarget: async (
    targetId: number | string,
    params: AdminHistoryByTargetParams,
  ): Promise<AdminHistoryPage> => {
    const response = await apiClient.get<AdminHistoryPageResponse>(`/api/v1/histories/${targetId}`, {
      params,
      paramsSerializer: {
        serialize: (rawParams) => {
          const searchParams = new URLSearchParams();
